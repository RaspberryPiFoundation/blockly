...
have registered your mutator, you can add it to your block's JSON definition. 
...
Blockly's default mutator UI is implemented in the `MutatorIcon` class. In most 
cases, you can use the default mutator UI that Blockly provides or create your 
own custom UI without instantiating `MutatorIcon` directly. If you are extending
Blockly itself to add new mutator features, you might want to extend the 
`MutatorIcon` class, but note that this will not always be needed. You can 
look at the [MutatorIcon](https://docs.blockly.com/reference/blockly.icons_namespace.mutatoricon_class/#iconsmutatoricon-class)
