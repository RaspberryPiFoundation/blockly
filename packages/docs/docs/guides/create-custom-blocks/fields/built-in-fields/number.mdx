...
A number field's ARIA label is `number: <ARIA value>`. A number input's 
on-block display has the ARIA role `button`. Activating the field overlays and
focuses an HTML text input.

:::note
The HTML input type is `text` rather than `number`. [According to MDN](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/input/number#accessibility)
