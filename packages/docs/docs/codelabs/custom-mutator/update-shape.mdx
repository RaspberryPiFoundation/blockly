    ...
    updateShape: function() {
        ...
        for (let i = 1; i < this.itemCount; i++) {
            if (!this.getInput('ADD' + i)) {
                this.appendValueInput('ADD' + i).setAriaLabelProvider(() => 'value ' + (i + 1));
            }
        }
    },
};
...
/* TODO: double check that this actually works properly with screenreaders */
...
Although `updateShape()` is completed, only the deserializer calls it. The next 
